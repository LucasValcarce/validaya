...
export default function BiometricCamera({ onSuccess, onCancel }) {
  ...
  /* Detiene TODOS los tracks del stream y limpia referencias */
  ...
  /* Inicia cámara al montar, la detiene SIEMPRE al desmontar */
  useEffect(() => {
    startCamera()
    return () => stopCamera()   // ← se ejecuta al cerrar el modal
  ...
  const handleCapture = useCallback(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (count > 0) {
        ...
        setCountdown(null)
        setPhase('success')
        stopCamera()   // ← detiene cámara inmediatamente tras captura
        ...
        <div className="relative bg-navy aspect-[4/3] flex items-center justify-center overflow-hidden">
          ...
          {(phase === 'ready' || phase === 'scanning') && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className={`relative w-52 h-52 transition-all duration-500 ${phase === 'scanning' ? 'scale-105' : ''}`}>
        ...
        <div className="px-5 py-4">
          {phase === 'error' ? (
            <div className="flex gap-3">
              <button onClick={handleCancel} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:border-gray-300 transition-colors">
                Cancelar
              </button>
              <button onClick={handleRetry} className="flex-1 py-3 rounded-xl bg-teal text-white font-semibold text-sm hover:bg-teal-hover transition-colors">
                Reintentar
              </button>
            </div>
          ) : phase === 'success' ? (
            <p className="py-2 text-center text-sm text-teal font-semibold">Procesando…</p>
          ) : (
            <div className="flex gap-3">
              <button onClick={handleCancel} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:border-gray-300 transition-colors">
                Cancelar
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2
                  ${phase === 'ready' ? 'bg-teal text-white hover:bg-teal-hover shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
              >
                {phase === 'scanning' ? (
                  <>
